...
function getSecureRandomItem<T>(array: T[]): T {
	...
	// Find the largest multiple of array.length that fits in a uint32
	const maxValidValue = Math.floor((2 ** 32 - 1) / array.length) * array.length;
	...
	} while (randomValue >= maxValidValue); // Reject values that would introduce bias
...
function generateXkcdPassword(wordCount = 4, separator = "-"): string {
	const words: string[] = [];
	...
	for (let i = 2; i < wordCount; i++) {
		...
		words.push(
			i % 2 === 0
				? getSecureRandomItem(ADJECTIVES)
				: getSecureRandomItem(NOUNS),
		);
	}
...
export class Password extends OpenAPIRoute {
	schema = {
		tags: ["Password"],
		summary:
			"Generate a random password using the XKCD 'correct horse battery staple' technique",
		description:
			"Creates a memorable password by combining random words, as popularized by XKCD comic #936",
		parameters: {
			wordCount: z
				.number()
				.int()
				...
				.max(10)
				.optional()
				.describe("Number of words to include in the password (default: 4)"),
			separator: Str({ example: "-" })
				.optional()
				.describe("Character to use between words (default: '-')"),
		},
		responses: {
			"200": {
	...
	async handle(c: Context) {
		...
		const wordCount = c.req.query("wordCount")
			? Number.parseInt(c.req.query("wordCount") as string, 10)
			: 4;
